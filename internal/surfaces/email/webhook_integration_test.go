...
// setupEmailEnv mirrors internal/surfaces/telegram's own setupTelegramEnv
// (this codebase's established per-file duplication idiom for integration
// test scaffolding).
func setupEmailEnv(t *testing.T) *pgxpool.Pool {
	t.Helper()
...
func insertTestTenant(t *testing.T, s *store.Store, tenantID uuid.UUID) {
	...
	if err := s.InTenantTx(context.Background(), tenantID, func(ctx context.Context, tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `INSERT INTO tenants (tenant_id, name) VALUES ($1, 'email-test')`, tenantID)
		return err
	}); err != nil {
...
func postEmailWebhook(t *testing.T, srv *Server, tenantID uuid.UUID, body []byte) *httptest.ResponseRecorder {
	t.Helper()
	req := httptest.NewRequest(http.MethodPost, "/v1/webhooks/email/"+tenantID.String(), bytes.NewReader(body))
...
// internal/surfaces/telegram's own (its doc comment). The key is
// lower-cased so a sender's address casing never splits the conversation.
func TestHandleWebhook_CreatesConversationalSessionWithDeterministicKey(t *testing.T) {
	...
	if sessionKey != "email:user@example.com" {
		t.Fatalf("session_key = %q, want %q", sessionKey, "email:user@example.com")
	}
	if !conversational {
...
func TestHandleWebhook_SecondMessageFromSamePeerResumesSameSession(t *testing.T) {
	...
	srv := &Server{
		...
		Channels: fakeChannels{user: "bot", pass: "secret", ok: true},
	}

	rec := postEmailWebhook(t, srv, tenantID, newEmailWebhookBody(t, "user@example.com", "first message"))
	if rec.Code != http.StatusOK {
	...
	// Casing differs from the first message on purpose — the lookup key
	// must be case-insensitive.
	rec = postEmailWebhook(t, srv, tenantID, newEmailWebhookBody(t, "User@Example.com", "second message"))
	if rec.Code != http.StatusOK {
